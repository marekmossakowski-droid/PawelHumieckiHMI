...
class PhysicalPrototypePersistenceReportingTests(unittest.TestCase):
    def test_round_trip_restart_recovery_and_report_from_committed_record(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            session = Session(
                session_id="P6-SYN-001",
                state=SessionState.IN_PROGRESS,
                identity=AnimalIdentityResolution(status=IdentityStatus.CONFIRMED, animal_id="SYN-COW-001"),
                animal_id="SYN-COW-001",
                treatment_refs=("hoof-trim",),
                material_refs=("dressing-1",),
                media_refs=("REF:synthetic-before.jpg",),
            )
            validator.commit_session(session)
            recovered = validator.recover_session("P6-SYN-001")
            self.assertEqual(recovered, session)

            pdf = validator.generate_report(
                "P6-SYN-001",
                report_id="P6-REPORT-001",
                generated_at=datetime(2026, 8, 21, 19, 40, 0),
                lesion_summary="synthetic lesion",
                treatment_summary="synthetic hoof trim",
