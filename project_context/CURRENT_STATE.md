...
## Current workstream
`HC-P7-001 — Physical prototype acceptance and closure readiness`

TDD lineage:
- RED head `74ad0ed8833eb686085c21d1470756eb922487a6` — acceptance tests failed before `hoofcare.physical.acceptance` existed;
- GREEN head `dc54408931605e5bfac95ebeb056b669d0f50563` — runtime-ci and docs-ci succeeded before reconciliation.

## P7 invariants
- acceptance requires a completed session with confirmed synthetic identity;
- committed session must round-trip through durable local storage unchanged;
- canonical PDF must preserve `source_session_id` and synthetic-test-only provenance;
- unresolved/incomplete sessions fail closed;
- machine-control-like actions remain explicitly rejected;
- `kvk_connection_allowed = false` and `real_farm_data_allowed = false` remain invariant.

## Closure readiness
P1-P7 form the complete isolated physical-prototype evidence set. Controlled merge and post-merge repository verification of P7 may establish `PHYSICAL PROTOTYPE = CLOSURE READY`, but it does not authorize live KVK integration, field deployment, real-farm data or production release.

## Next dependency-ordered step
After controlled merge and repository verification of P7, create the formal physical-prototype closure/reconciliation record and prepare any later live observation-only authority as a separate Project Owner decision.
